...
namespace MACS_Game
{
    class ExcelWrite
    {
        public String FolderName { get; private set; } = DateTime.Now.ToString("ddMMyyyyhhmmssff");
        ...
        public void OutputAgentType(int iteration, IEnumerable<Agent> ActiveAgents)
        {
            using (SpreadsheetDocument document = SpreadsheetDocument.Create(FolderName + $"/OutputAgentType/Iteration{iteration}.xlsx", SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (Agent agent in ActiveAgents)
                {
                    row = new Row();
                    sheetData.Append(row);
                    InsertCell(row, agent.Name, CellValues.String);
                    InsertCell(row, agent.Contrib ? "Contrib" : "NonContrib", CellValues.String);
                }
                        ...
                        for (int r = row - 1; r <= row + 1; r++)
                        {
                            ...
                            for (int c = col - 1; c <= col + 1; c++)
                            {
                                ...
                                if (socialspace[rowNeighbourSpot, colNeighbourSpot] != null)
                                    relations.Add((socialspace[row, col].Name, socialspace[rowNeighbourSpot, colNeighbourSpot].Name));
                            }
                        }
                    }
